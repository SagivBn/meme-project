...
function onSelectNav(nav, prevNav) {
    ...
    document.querySelector(`.${prevNav}-container`).classList.remove(`nav-selected`);
    document.querySelector(`.${nav}-container`).classList.add('nav-selected');
}

function onGalleryFilter(elInput) {
...
function onRenderKeywordsList() {
    ...
    if (showMore) {
        for (let i = 0; i < 3; i++) {
            ...
            keywords.slice(i * 7, (i + 1) * 7).forEach(keyword => (keywordsTotalValue += keyword.value));
            strHTML = keywords.map(
                keyword => `
    <li style="font-size: ${(keyword.value * 6) / keywordsTotalValue}em;" onclick="onSetImgFilter(undefined, this)">${
                    keyword.name.charAt(0).toUpperCase() + keyword.name.slice(1)
                }</li>
    `,
            );
        }
        document.querySelector('.searched-key-words').innerHTML = `<ul>${strHTML.slice(0, 7).join('')}</ul><ul>${strHTML
            .slice(7, 14)
        ...
        strHTML = keywords.map(
            keyword => `
    <li style="font-size: ${(keyword.value * 10) / keywordsTotalValue}em;" onclick="onSetImgFilter(undefined, this)">${
                keyword.name.charAt(0).toUpperCase() + keyword.name.slice(1)
            }</li>
    `,
        );
        document.querySelector('.searched-key-words').innerHTML = `<ul>${strHTML.join('')}</ul>`;
    }
}
